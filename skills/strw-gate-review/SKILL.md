...
5. Запис у git завершує контур M — `strw-run` або сама сесія на Mac, повідомлення `gate(<G#>, <product>): <рішення>`. `commit-on-stop.sh` коміта не робить: він лише не дає завершити сесію з незаписаним станом. У контурі C — файл у `_outbox/`.
