...
### Step 4 — Maker phase
Виклич maker-агента з паспорта (Task tool, subagent із `agents/`). Передай йому: scope, релевантні файли state, контракт вихідного артефакту (references/artifact-contracts.md). Fan-out — за references/subagent-delegation.md (≤6).

### Step 5 — Checker phase
Виклич checker-агента з паспорта (ЗАВЖДИ інший, ніж maker). Вердикт FAIL → поверни maker'у (max 2 ітерації), далі — розбіжність фіксується в артефакті, ескалація.
