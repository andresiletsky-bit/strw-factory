...
budget: maker — opus; checker — ІНША модель, ніж maker (hard rule, budget-policy); fan-out ≤6 haiku/sonnet; 3 запуски/тиждень max
...
4. **Prototype ≠ production:** усе, що зібрано для валідації (лендінг, скрипт, мок), — disposable: живе в `products/<id>/prototypes/` або окремій гілці, позначене PROTOTYPE. Перенесення в пост-G2 кодову базу — ТІЛЬКИ через повний code-review (частіше — переписати). Прототипи не шипляться «випадково».
