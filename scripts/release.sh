...
if [ -n "$(printf '%s' "$MESSAGE" | tr -d '[:space:]')" ]; then
  ...
  # Один прохід perl: (1) інлайн-код-спани `…` (лише в межах рядка — непарний
  # бектик не розтягується через рядки) ховаються у плейсхолдери; (2) парність
  # <!-- / --> рахується на тексті БЕЗ спанів — непарний = секція зламана, код 3;
  # (3) коментарі знімаються з тексту без спанів; (4) спани повертаються на місце
  # дослівно — `<!-- … -->` у бектиках лишається в нотатках (це текст про маркер).
  # Межа: лише інлайн-спани в одинарних бектиках; fenced-блоки ``` і подвійні
  # бектики не розпізнаються — коментар усередині них зріжеться, незакритий
  # дасть відмову (fail-closed, з причиною).
  # Перша редакція (#20 р.1) знімала спани лише для лічильника, а зріз робила по
  # сирому тексту — бектиковий <!-- з'їдав текст до справжнього -->.
  # `|| PRC=$?`, бо під set -e падіння підстановки в присвоєнні вбило б скрипт
  # кодом perl (3) без названої причини.
  PRC=0; NOTES_RAW="$(printf '%s\n' "$SECTION" | perl -e '
    local $/; my $t = <STDIN>; my @s; $t =~ s/\x01//g;   # службовий байт плейсхолдера — не з вхідного тексту
    $t =~ s/(`[^`\n]*`)/push @s, $1; "\x01" . $#s . "\x01"/ge;
    my $o = () = $t =~ m{<!--}g; my $c = () = $t =~ m{-->}g;
    exit 3 if $o != $c;
    $t =~ s/<!--.*?-->//gs;
    $t =~ s/\x01(\d+)\x01/$s[$1]/g;
    print $t;')" || PRC=$?
  [ "$PRC" -eq 3 ] && die "у секції [Unreleased] непарний HTML-коментар (<!-- без --> або навпаки, поза код-спанами) — виправ секцію перед релізом"
  [ "$PRC" -eq 0 ] || die "не вдалось очистити нотатки з CHANGELOG (perl rc=$PRC)"
  printf '%s\n' "$NOTES_RAW" | sed '/^[[:space:]]*$/d' > "$NOTES_FILE"
fi
...
# в обидва боки (кеш→тег і тег→кеш); його ж кличе preflight strw-ops/bin/strw-run.sh
# щотіку (strw-ops a6e2231), тож реліз і раннер міряють одну рівність одним кодом.
