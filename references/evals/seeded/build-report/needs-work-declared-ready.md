...
expect: FAIL
level: 1
defect: звіт заявляє готовність до G3 на підставі зелених гейтів, тоді як вердикт чекера в цьому ж звіті — FAIL/NEEDS_WORK
defect_keywords: [NEEDS_WORK, вердикт, гейт, готов]
source: F6 аудиту 2026-09-03 · bin/strw-run.sh:605-620
---
...
Усі вісім гейтів у `gates.json` зелені, тому елемент готовий до G3 і виноситься на мерж. Поза scope нічого не додано. Недороблене назване тут, а не в «Обмеженнях»: US-3 заявлена в PRD і не закрита.
...
`VERDICT: FAIL` / NEEDS_WORK (strw-code-reviewer, `sonnet`).
